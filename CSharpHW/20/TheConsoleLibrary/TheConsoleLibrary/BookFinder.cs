...
namespace TheConsoleLibrary
{
    public class BookFinder
    {
        ...
        public void Find(string input)
        {
            ...
            if (target.Length == 1)
            {
                var books = Library.Shelf.Where(book => book.Title.Contains(target[0].Trim()) || book.Author.Contains(target[0].Trim()));
                BooksFound.AddRange(books);
            }
            else if (target.Length == 2)
            {
                var books = Library.Shelf.Where(book => book.Title.Contains(target[0].Trim()) && book.Author.Contains(target[1].Trim()));
                BooksFound.AddRange(books);
            }
